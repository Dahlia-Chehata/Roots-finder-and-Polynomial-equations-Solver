...
try
    ...
    bisecRoot = biiterTable(1:size(biiterTable), 5:5);
    bisecError = biiterTable(1:size(biiterTable), 7:7);
    regRoot = rfiterTable(1:size(rfiterTable), 3:3);
    regError = rfiterTable(1:size(rfiterTable), 5:5);
    illRoot = illIterable(1:size(illIterable), 3:3);
    illError = illIterable(1:size(illIterable), 5:5);
    fixedRoot = fpiterTable(1:size(fpiterTable), 2:2);
    fixedError = fpiterTable(1:size(fpiterTable), 4:4);
    newtRoot = npiterTable(1:size(npiterTable), 4:4);
    newtError = npiterTable(1:size(npiterTable), 5:5);
    secRoot = seciterTable(1:size(seciterTable), 3:3);
    secError = seciterTable(1:size(seciterTable), 6:6);
    birgeRoot = bviterTable(1:size(bviterTable), 2:2);
    birgeError = bviterTable(1:size(bviterTable), 3:3);
    plotAllIterations(bisecRoot, regRoot, fixedRoot, newtRoot, secRoot, birgeRoot, illRoot, handles);
    plotAllError(bisecError, regError, fixedError, newtError, secError, birgeError, illError, handles);
catch ME
